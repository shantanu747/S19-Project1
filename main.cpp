  ...
  while(int(serviceQ.size()) != numProcesses)
  {
     ...
     if(cpuInUse && time == burst_end)
     {
       ...
       else if(all_p[cp].getNumBursts() > 0)
       {
         ...
         if(time <= 999)
         {
           if(all_p[cp].getNumBursts() == 1)
           {
             cout << "time " << time << "ms: Process " << all_p[cp].getID() << " completed a CPU burst; " << all_p[cp].getNumBursts() << " burst to go ";
           }
...
void RR(vector<Process> p, int n, int switch_time, int tslice, string behavior)
{
    ...
    while (serviceQ.size() != all_p.size())
    {
      ...
      if(!cpuInUse && time == startNextProcess)
      {
        if(all_p[cp].getID() == readyQ[0].getID() && readyQ.size()>0)
        {
        ...
        else
        {
          ...
          if(time <= 999)
          {
            cout << "time " << time << "ms: Process " << all_p[cp].getID() << " started using the CPU for " << all_p[cp].getRemainingTimeInBurst()+t_slice << "ms burst ";
            printQ_RR(readyQ);
          }
        }
    ...
    outfile << "-- average turnaround time: " << std::fixed << setprecision(3)<<totalTurnaroundTime/totalBursts << "ms" << endl;
    outfile << "-- total number of context switches: " << contextSwitches << endl;
    outfile << "-- total number of preemptions: " << totalPreempt << endl;
    outfile.close();
}
